    ...
    "columnDefs" : [
      {
        ...
        "render" : function (data, type, row) {
          ...
          // 编辑页面
          var yaml = '/kubernetes/' + cluster + '/' + namespace + '/' + row.name + '/change_daemonset';
          var image = '/kubernetes/' + cluster + '/' + namespace + '/' + row.name + '/images_daemonset';
          // 操作按钮
          var html = "<a class='btn btn-primary btn-xs btn_margin' href='" + image + "'>更新镜像</a>";
          html += "<a class='btn btn-warning btn-xs btn_margin info_margin' href='" + yaml + "'>更新YAML</a>";
          html += "<button type='button' id='daemonset_del' class='btn btn-danger btn-xs btn_margin info_margin'>删除</button>";
          return html;
        },
      }
    ],
...
$(function () {
  // 默认加载数据
  url = '/kubernetes/' + $('#cluster').val() + '/' + $('#namespace').val() + '/read_daemonset';
  daemonset(url);
  // 根据集群加载默认数据
...
// daemonset delete
$('#daemonset_table').on('click', 'button#daemonset_del',function () {
  var data = $("#daemonset_table").DataTable().row( $(this).parents('tr') ).data();
  bootbox.confirm({
    title: "提示内容:",
    ...
    callback: function (result) {
      ...
      if (result) {
        $.ajax({
          url: '/kubernetes/' + cluster + '/' + namespace + '/' + data.name + '/delete_daemonset',
          type: "post",
          data: {
