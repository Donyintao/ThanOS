    ...
    "columnDefs" : [
      {
        ...
        "render" : function (data, type, row) {
          ...
          // 编辑页面
          var yaml = '/kubernetes/' + cluster + '/' + namespace + '/' + row.name + '/change_statefulset';
          var image = '/kubernetes/' + cluster + '/' + namespace + '/' + row.name + '/images_statefulset';
          var replicas ='/kubernetes/' + cluster + '/' + namespace + '/' + row.name + '/replicas_statefulset';
          // 操作按钮
          ...
          html += "<button type='button' id='statefulset_del' class='btn btn-danger btn-xs btn_margin info_margin'>删除</button>";
          return html;
        },
      }
    ],
...
$(function () {
  // 默认加载数据
  url = '/kubernetes/' + $('#cluster').val() + '/' + $('#namespace').val() + '/read_statefulset';
  statefulset(url);
  // 根据集群加载默认数据
...
// statefulset delete
$('#statefulset_table').on('click', 'button#statefulset_del',function () {
  var data = $("#statefulset_table").DataTable().row( $(this).parents('tr') ).data();
  bootbox.confirm({
    title: "提示内容:",
    ...
    callback: function (result) {
      ...
      if (result) {
        $.ajax({
          url: '/kubernetes/' + cluster + '/' + namespace + '/' + data.name  + '/delete_statefulset',
          type: "post",
          data: {
...
$(function () {
  ...
  $('.up_imageValid').click(function () {
    ...
    if ($('#up_imageForm').data("bootstrapValidator").isValid()) {
      $.ajax({
        url: '/kubernetes/' + path[1] + '/' + path[2] + '/' + path[3] + '/images_statefulset',
        type: "post",
        data: {
          image: $('#image').val(),
...
$(function () {
  ...
  $('.up_resValid').click(function () {
    ...
    if ($('#up_resForm').data("bootstrapValidator").isValid()) {
      $.ajax({
        url: '/kubernetes/' + path[1] + '/' + path[2] + '/' + path[3] + '/replicas_statefulset',
        type: "post",
        data: {
